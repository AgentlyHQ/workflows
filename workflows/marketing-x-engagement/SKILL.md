...
- `sort`: `recent`
- `timeRange`: `week`. If the agent lacks `timeRange`, still request a timestamp field (e.g., `posted_at` or `created_at`) and, after the search returns, locally drop any tweet older than 7 days before selecting the most recent items.
...
- If fewer than `${TWEET_LIMIT}` rows are available, add a single line immediately under the table in this exact format (replace `N` with the actual count you found): `_Only N of ${TWEET_LIMIT} recent tweets found (last 7 days)._`
