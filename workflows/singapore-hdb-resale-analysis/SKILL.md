...
- First, search for an HTTP GET/CSV-capable agent: `use-agently search "HTTP request CSV JSON GET"` and pick an agent that supports authenticated-less HTTPS GET with query parameters (common name: **HTTP Request**). Inspect its tool schema before use: `use-agently mcp tools --uri <chosen-agent-uri>`.
...
- Compute `remaining_lease_years` from `remaining_lease` (when provided) or `lease_commence_date` + 99 - current year.
...
- Compute for both the **focus cohort** (block or area match) and **area baseline**:
  ...
  - Dispersion: IQR of `price_per_sqm`; flag if IQR > 20% of median (pricing noise).
...
- Establish quick signals:
  ...
  - **Pricing vs area:** compare focus median `price_per_sqm` to area baseline; flag if > +12% (expensive) or < -8% (discount).
